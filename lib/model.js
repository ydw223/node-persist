...
function addBelongsToAssociationMethod (obj, associationName, association) {
  obj.__defineGetter__(associationName, function () {
    if (!doesObjectHaveAConnection(obj)) {
      return [];
    }
...
function addAssociationMethod (obj, associationName, association) {
  switch (association.type) {
    ...
    case "hasOne":
      ...
      break;
    case "belongsTo":
      addBelongsToAssociationMethod(obj, associationName, association);
      break;
    default:
...
exports.define = function (name, columnDefs, opts) {
  ...
  Model.hasMany = function (associatedModel, opts) {
    ...
    var name = opts.name || inflection.camelize(inflection.underscore(inflection.pluralize(associatedModel.modelName)), true);
  ...
  Model.hasOne = function (associatedModel, opts) {
    ...
    var name = opts.name || inflection.camelize(inflection.underscore(inflection.singularize(associatedModel.modelName)), true);
    if (this.associations[name]) {
      return this;
    }
    this.associations[name] = opts;
  ...
  Model.normalizeBelongsToOptions = function (associatedModel, opts) {
    var foreignKey = inflection.foreignKey(inflection.singularize(associatedModel.modelName));
    opts = opts || {};
    opts.type = "belongsTo";
    opts.model = associatedModel;
  ...
  Model.belongsTo = function (associatedModel, opts) {
    opts = this.normalizeBelongsToOptions(associatedModel, opts);
    var name = opts.name || inflection.camelize(inflection.underscore(inflection.singularize(associatedModel.modelName)), true);
    if (this.associations[name]) {
      return this;
    }
    ...
    if(!opts.hasOwnProperty('createHasOne') || opts.createHasOne) {
      var associatedOpts = persistUtil.shallowCopy(opts);
      associatedOpts.createHasMany = false;
