...
var SqlTree = Class.extend({
  ...
  populateInclude: function (result, item, include, start_col, end_col) {
    ...
    if (include.association.type === 'hasOne' || include.association.type == 'belongsTo') {
  ...
  populateIncludes: function (result, item, start_col) {
    ...
    for (i = 0; i < this.includes.length; i++) {
      ...
      end_col = Number(include.primaryKeyColumn.alias.substring(1))+Object.keys(include.association || {}).length;
  ...
  toObjects: function (rows) {
    ...
    if (this.primaryKeyColumn) {
      ...
      for (i = 0; i < items.length; i++) {
        ...
        this.populateIncludes(result, item, Number(this.primaryKeyColumn.alias.substring(1))+Object.keys(include.association || {}).length);
      }
      return results;
    }
